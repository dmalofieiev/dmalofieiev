<h1 align="center">Hi there, I'm <a href="" target="_blank">Dmytro!</a>
<img src="https://github.com/blackcater/blackcater/raw/main/images/Hi.gif" height="32"/></h1>

<h3 align="center">Front-End Developer, specializing in designing user-friendly, cross-browser compliant websites and UI applications.</h3>

---

<h3 align="center">💻 My Technology Stack</h3>

<table align="center" style="width: 100%">
  <tr>
    <th>Front-End</th>
    <th>Back-End</th>
    <th>Tools</th>
  </tr>
  <tr>
    <td valign="top">
      <ul>
        <li><img src="img/javascript-color.svg" height="20" alt="JavaScript"/> JavaScript</li>
        <li><img src="img/typescript-color.svg" height="20" alt="TypeScript"/> TypeScript</li>
        <li><img src="img/react-color.svg" height="20" alt="React"/> React</li>
        <li><img src="img/redux-color.svg" height="20" alt="Redux"/> Redux Toolkit</li>
        <li><img src="img/vue-svgrepo-com (1).svg" height="20" alt="Vue.js"/> Vue.js</li>
        <li><img src="img/html5-color.svg" height="20" alt="HTML5"/> HTML5</li>
        <li><img src="img/css3-color.svg" height="20" alt="CSS3"/> CSS3</li>
        <li><img src="img/scss2-svgrepo-com.svg" height="20" alt="SCSS"/> SCSS</li>
      </ul>
    </td>
    <td valign="top">
      <ul>
        <li><img src="img/nodedotjs-color.svg" height="20" alt="Node.js"/> Node.js</li>
        <li><img src="img/express-color.svg" height="20" alt="Express"/> Express</li>
        <li><img src="img/postgresql-color.svg" height="20" alt="PostgreSQL"/> PostgreSQL</li>
        <li><img src="img/telegram-svgrepo-com.svg" height="20" alt="Telegram API"/> Telegram API</li>
      </ul>
    </td>
    <td valign="top">
      <ul>
        <li><img src="img/git-svgrepo-com.svg" height="20" alt="Git"/> Git</li>
        <li><img src="img/bootstrap-color.svg" height="20" alt="Bootstrap"/> Bootstrap</li>
        <li><img src="img/mui-color.svg" height="20" alt="MUI"/> MUI</li>
        <li><img src="img/figma-color.svg" height="20" alt="Figma"/> Figma</li>
      </ul>
    </td>
  </tr>
</table>

